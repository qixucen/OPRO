...
def run_evolution(**kwargs):
    ...
    evaluate_generated_ins_on_few_shot = kwargs["evaluate_generated_ins_on_few_shot"]
    ...
    for i_step in range(num_search_steps):
                ...
                if optimizer_llm_name.lower() in {"gpt-3.5-turbo", "gpt-4"}:
                    ...
                    for raw_output in raw_outputs:
                        if start_string not in raw_output:
                            ...
                            start_index = raw_output.index(start_string) + len(
                                start_string
                            )
        ...
        for instruction in generated_instructions:
            if len(instruction) > 500:
                ...
                continue
            if dataset_name == "gsm8k" and any(char.isdigit() for char in instruction):
                print(
